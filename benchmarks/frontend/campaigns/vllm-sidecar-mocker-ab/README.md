...
- Live trajectory-tree concurrency `{1024,4096,8192,16384}`.
- Four 900-second legs per point in crossover order `direct, sidecar, sidecar, direct`.
- One 16,384-concurrency qualification leg before measurement. It rejects load-generator CPU, trajectory realization, FD, socket, or admission-queue contamination. Preserved `c131072`, `c65536`, and `c32768` pre-campaign attempts saturated the 64 load-generator cores, so the locked matrix stops at the largest planned concurrency this fixed host allocation can validly generate.
...
- Sidecar pool sweep `{8,16,32,64,128}` ascending and descending at concurrency 16,384 and the measured sidecar capacity peak. Valid eight-connection main legs are reused.
