...
- Live trajectory-tree concurrency `{512,1024,2048}`.
- Four 900-second legs per point in crossover order `direct, sidecar, sidecar, direct`.
- One 2,048-concurrency qualification leg before measurement. It rejects load-generator CPU, trajectory realization, FD, socket, or admission-queue contamination. Preserved `c131072`, `c65536`, `c32768`, `c16384`, `c8192`, and `c4096` attempts saturated the load path or returned no successful profiling requests, so the locked matrix stops at the largest concurrency that can still produce a measurable A/B distribution on this fixed host allocation.
