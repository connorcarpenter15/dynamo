...
- One 8,192-concurrency qualification leg before measurement. It rejects load-generator CPU, trajectory realization, FD, socket, or admission-queue contamination. Preserved `c2048`, `c4096`, and `c8192` AgentX attempts using the original 4,194,304-block synthetic fixture were invalid because 262,144-token traces made simulated KV capacity the scheduler bottleneck. The locked fixture now provides 67,108,864 blocks and a 2,147,483,648-token batch budget, enough for every c8192 lane at the maximum accepted context length.
