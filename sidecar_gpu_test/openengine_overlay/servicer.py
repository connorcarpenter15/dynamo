...
class OpenEngineServicer(pb_grpc.OpenEngineServicer):
    ...
    async def SubscribeKvEvents(self, request, context):
        ...
        await context.abort(
            _UNIMPLEMENTED, "SubscribeKvEvents not implemented in v1; use GetKvEventSources"
        )
        return
        yield  # pragma: no cover - makes this an async generator

    async def SubscribeRuntimeEvents(self, request, context):
        await context.abort(_UNIMPLEMENTED, "SubscribeRuntimeEvents not implemented in v1")
        return
